...
CREATE TABLE "dataFeeds" (
    ...
    "schemaPluginId"    uuid,
    "mapperPluginId"    uuid,
    "storagePluginId"   uuid,
    "outpostPluginId"   uuid,
    ...
    CONSTRAINT "dataFeeds_mapperPluginId_fkey"
        ...
        ON UPDATE CASCADE ON DELETE SET NULL,
    CONSTRAINT "dataFeeds_storagePluginId_fkey"
        FOREIGN KEY ("storagePluginId") REFERENCES plugins("pluginId")
        ON UPDATE CASCADE ON DELETE SET NULL,
    CONSTRAINT "dataFeeds_outpostPluginId_fkey"
        FOREIGN KEY ("outpostPluginId") REFERENCES plugins("pluginId")
        ON UPDATE CASCADE ON DELETE SET NULL,
...
CREATE INDEX idx_datafeeds_storage_plugin  ON "dataFeeds" ("storagePluginId")   WHERE "storagePluginId" IS NOT NULL;
CREATE INDEX idx_datafeeds_outpost_plugin  ON "dataFeeds" ("outpostPluginId")   WHERE "outpostPluginId" IS NOT NULL;
...
BEGIN
    ...
    (v_plugins, 'plugins', 'archive', 'Internal',
     '{"$schema":"https://json-schema.org/draft/2020-12/schema","title":"plugins Schema","description":"From Internal","type":"object","properties":{"pluginName":{"type":"string","maxLength":255,"title":"pluginName"},"pluginVersion":{"type":"string","maxLength":255,"title":"pluginVersion"},"pluginType":{"type":["object","enum"],"title":"pluginType","x-zodType":"options","x-zodOptions":[["validation","validation"],["mapping","mapping"],["storage","storage"],["outpost","outpost"]],"enum":["validation","mapping","storage","outpost"]},"pluginMinioObjectPath":{"type":["object","string"],"title":"pluginMinioObjectPath","x-zodType":"file","x-zodFile":{"file":"","key":"pluginMinioObjectPath","content":"","mimes":[".js",".ts",".json"]}},"securityLevel":{"type":["object","enum"],"title":"securityLevel","x-zodType":"options","x-zodOptions":[["low","low"],["medium","medium"],["high","high"]],"enum":["low","medium","high"]},"config":{"type":"string","title":"config"},"notes":{"type":"string","maxLength":2000,"title":"notes"}},"required":["pluginName","pluginVersion","pluginType","pluginMinioObjectPath","securityLevel"],"additionalProperties":false}'::json,
     true, NOW(), NOW()),
    ...
    (v_formschemas, 'formSchemas', 'archive', 'Internal',
     '{"$schema":"https://json-schema.org/draft/2020-12/schema","title":"formSchemas Schema","description":"From Internal","type":"object","properties":{"schemaName":{"type":"string","maxLength":255,"title":"schemaName"},"schemaType":{"type":["object","enum"],"title":"schemaType","enum":["form","archive"],"x-zodType":"options","x-zodOptions":[["form","form"],["archive","archive"]]},"version":{"type":"string","maxLength":255,"title":"version"},"schema":{"type":"string","title":"schema"},"isActive":{"type":"boolean","title":"isActive"},"dataFeedId":{"title":"dataFeedId","type":["object","string"],"format":"uuid","x-zodType":"reference","x-zodReference":{"table":"dataFeeds","key":"dataFeedId","attributes":["dataFeedName","createdAt","updatedAt"]},"x-zodVisibility":{"conditions":[{"field":"schemaType","operator":"equals","value":"form"}],"logic":"and"}}},"required":["schemaName","schemaType","version","schema","isActive"],"additionalProperties":false,"allOf":[{"if":{"properties":{"schemaType":{"const":"form"}}},"then":{"required":["dataFeedId"]}}]}'::json,
     true, NOW(), NOW())
...
CREATE INDEX IF NOT EXISTS "idx_messageProcessingRuns_status_createdAt"
  ON "messageProcessingRuns" ("currentStatus", "createdAt" DESC);
...
-- ----------------------------------------------------------
-- Content-addressable source deduplication
-- ----------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS "idx_fileHashes_messageId"
  ON "fileHashes" ("messageId");

CREATE INDEX IF NOT EXISTS "idx_fileHashes_projectId"
  ON "fileHashes" ("projectId");
