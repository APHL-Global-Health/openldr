  ...
  if (Array.isArray(messageContent.susceptibility_tests)) {
    messageContent.susceptibility_tests.forEach((row: any, index: number) => {
      if (!row.antibiotic_concept_id)
        errors.push(
          `susceptibility_tests[${index}].antibiotic_concept_id is required`,
        );
      if (typeof row.isolate_index !== "number") {
        ...
        errors.push(
          `susceptibility_tests[${index}].isolate_index ${row.isolate_index} does not reference an isolate`,
        );
      }
    });
  }
...
export async function handleMessage(kafkaMessage: any) {
  try {
    ...
    if (messageId && await messageTrackingService.isRunDeleted(messageId)) {
      logger.info({ messageId }, "Skipping deleted run in storage");
      return;
    }
    ...
    if (messageId) {
      await messageTrackingService.markStageStarted({
        messageId,
        stage: "storage",
        topic: kafkaMessage.topic || "mapped-inbound",
        objectPath: `${projectId}/${mappedName}`,
        pluginName:
          dataFeed.storagePlugin?.pluginName ||
          dataFeed.storagePlugin?.pluginName ||
          null,
        pluginVersion:
          dataFeed.storagePlugin?.pluginVersion ||
          dataFeed.storagePlugin?.pluginVersion ||
          null,
      });
    }
    ...
    const { plugin, selection } = await pluginService.resolvePluginSelection({
      pluginID: dataFeed.storagePluginId,
      pluginType: "storage",
      pluginVersion: dataFeed.storagePlugin?.pluginVersion || null,
    });
