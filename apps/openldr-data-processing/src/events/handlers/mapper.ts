...
export async function handleMessage(kafkaMessage: any) {
  try {
    ...
    if (messageId && await messageTrackingService.isRunDeleted(messageId)) {
      logger.info({ messageId }, "Skipping deleted run in mapping");
      return;
    }
    ...
    if (messageId) {
      await messageTrackingService.markStageStarted({
        messageId,
        stage: "mapping",
        topic: kafkaMessage.topic || "validated-inbound",
        objectPath: `${projectId}/${validatedName}`,
        pluginName: dataFeed.mapperPlugin?.pluginName || null,
    ...
    if (messageId) {
      await messageTrackingService.markStageCompleted({
        ...
        pluginName:
          runtimePlugin.pluginName || dataFeed.mapperPlugin?.pluginName || null,
        pluginVersion:
          runtimePlugin.pluginVersion ||
          dataFeed.mapperPlugin?.pluginVersion ||
          null,
      });
    }
  } catch (error: any) {
