...
async function upsertLabRequest(
  ...
  const sql = `
    ...
    DO UPDATE SET
      ...
      requesting_facility = COALESCE(EXCLUDED.requesting_facility, lab_requests.requesting_facility),
      testing_facility = COALESCE(EXCLUDED.testing_facility, lab_requests.testing_facility),
  ...
  const result = await client.query(sql, [
    ...
    normalizeNullableString(request?.requesting_facility) || null,
    normalizeNullableString(request?.testing_facility) || null,
