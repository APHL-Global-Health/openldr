...
const PLUGIN_TIMEOUT_MS = 120_000; // 2 minutes — large WHONET files (28K rows + AMR) need time
...
async function execPluginCode(
  ...
  if (type === "validation") {
    ...
    vmExec = `
      if (typeof process !== 'function') throw new Error('Storage plugin must export a function named \\'process\\'');

      result = { output: process(payload) };
    `;
...
export async function runPluginTest(
  ...
  if (opts.validation) {
    try {
      ...
      stages.storage = {
        output: {},
        logs: ["Validation failed, storage not ran"],
        durationMs: 0,
      };
      stages.outpost = {
        output: {},
  ...
  if (allPassed && opts.mapping) {
    try {
      ...
      stages.storage = {
        output: {},
        logs: ["Mapping failed, storage not ran"],
        durationMs: 0,
      };
      stages.outpost = {
        output: {},
  ...
  // ── Storage ──
  if (allPassed && opts.storage) {
    try {
      const sResult = await runOutpost(
        opts.storage.type,
        opts.storage.code,
        currentPayload,
      );
      stages.storage = sResult;
      currentPayload = sResult.output;
    } catch (err) {
      stages.storage = {
        output: {},
        logs: [(err as Error).message],
