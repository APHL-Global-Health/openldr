...
else app.use(morgan('combined', { stream: { write: (message: string) => logger.info(message.trim()) } }));
app.use('/api/v1', dataProcessingRouter);
app.get('/health', (_req: Request, res: Response) => {
  res.json({ status: 'ok', timestamp: new Date().toISOString(), uptime: formatUptime(process.uptime()) });
});
app.get('/', (_req: Request, res: Response) => {
  res.json({ message: 'Data Processing API', version: '1.3.0', health: '/health', endpoints: { api: '/api/v1' } });
});
