// OpenLDR Disalab Data Recipient Plugin
// Plugin to store disalab lab data in the lab data database
...
async function apiCall(endpoint, method = 'GET', data = null) {
  ...
  try {
    const headers = {
      'User-Agent': 'OpenLDR-Disa-Plugin/1.0.0'
    };
  ...
  try {
    ...
    for(let xx=0; xx<items.length; xx++){
      ...
      if(item){
        ...
        const patientResponse = await apiCall('/api/v1/patients', 'POST', patient);
        ...
        for(let z=0; z<orders.length; z++){
          ...
          const requestData = {
            ...
            request_data: {
                ...
                rejection_code: null, 
                requesting_lab_code: item?.Facility?.Code ? `DISA${item?.Facility?.Code}` : null,
