...
type DateTimeInputProps = {
  ...
  // onChanging?: (value?: string | undefined | null) => void;
  format?: string;
  disabled?: boolean;
  clearable?: boolean;
  timezone?: string;
...
const DateTimeInput = React.forwardRef<HTMLInputElement, DateTimeInputProps>(
  (options: DateTimeInputProps, ref) => {
    ...
    useEffect(() => {
      if (!inputValue) {
        //update data with undefined
        // options.onChanged?.(inputValue);
      } else if (value?.getTime() !== inputValue.getTime()) {
        options.onChanged?.(inputValue);
      }
    ...
    //     // onChanging?.(inputRef.current.value);
    //   }
    // }, [inputRef?.current?.value, onChanged, segments]);
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "flex flex-1 items-center justify-start rounded-md border border-input bg-background dark:bg-input/30 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground  disabled:cursor-not-allowed disabled:opacity-50",
      ...
      >
        ...
        <input
          ...
          className="border-none focus:outline-none focus:ring-0  font-mono flex-grow min-w-0 bg-transparent text-sm disabled:cursor-not-allowed disabled:opacity-50"
        ...
        <div>
          {inputValue ? (
            <CalendarIcon className="size-4 text-foreground-default" />
          ) : (
            <TooltipProvider>
              <Tooltip>
