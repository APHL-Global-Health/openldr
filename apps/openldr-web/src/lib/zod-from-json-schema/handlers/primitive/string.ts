  ...
  apply(types: TypeSchemas, schema: JSONSchema.BaseSchema): void {
    ...
    if (
      ...
      (stringSchema.minLength !== undefined ||
        ...
        stringSchema.pattern !== undefined ||
        stringSchema.format !== undefined)
    ) {
...
export class FormatHandler implements PrimitiveHandler {
  apply(types: TypeSchemas, schema: JSONSchema.BaseSchema): void {
    const stringSchema = schema as JSONSchema.StringSchema;
    if (!stringSchema.format) return;
