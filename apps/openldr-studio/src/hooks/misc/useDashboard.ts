  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (state.activeTab === "laboratory") {
        ...
        // Fire storage fetch separately — it's slow (MinIO object listing)
        // and shouldn't block the rest of the dashboard
        getInfraStorage(token, signal)
          .then((storageResult) => {
            if (!signal.aborted) {
              dispatch({ type: "SET_DATA", payload: storageResult });
            }
          })
          .catch(() => {
            // Storage failure shouldn't break the dashboard
          });
