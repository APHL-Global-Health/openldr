...
interface ModelState {
  ...
  // Actions
  startDownload: (modelId: string, filename?: string) => Promise<void>;
  pollDownloadStatus: (modelId: string) => Promise<void>;
  loadModel: (modelId: string, filename?: string) => Promise<void>;
...
export const useModelStore = create<ModelState>()(
  persist(
    (set, get) => ({
      ...
      startDownload: async (modelId: string, filename?: string) => {
        ...
        try {
          await aiClient.downloadModel(modelId, filename);
      ...
      loadModel: async (modelId: string, filename?: string) => {
        set({ isLoadingModel: true, loadError: null });
        try {
          await aiClient.loadModel(modelId, filename);
          // Auto-disable thinking if new model doesn't support it
          const { SUGGESTED_MODELS } = await import("@/components/chat/chat-model-selector");
          const supportsThinking = SUGGESTED_MODELS.find((m) => m.id === modelId)?.supportsThinking ?? false;
          set({
            loadedModelId: modelId,
            isLoadingModel: false,
            thinkingEnabled: supportsThinking ? get().thinkingEnabled : false,
          });
        } catch (err) {
          set({
            isLoadingModel: false,
