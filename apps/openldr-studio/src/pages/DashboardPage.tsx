          ...
          {data && activeTab === "laboratory" ? (
            ...
            <div className="space-y-6 p-6 min-h-[calc(100vh-26px-56px)] max-h-[calc(100vh-26px-56px)] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                ...
                {data.storage ? (
                  <StorageOverviewCard storage={data.storage} />
                ) : (
                  <div className="rounded-xl border bg-card p-6 flex flex-col items-center justify-center gap-2 min-h-60">
                    <div className="h-4 w-4 rounded-full border-2 border-muted-foreground/30 border-t-muted-foreground animate-spin" />
                    <span className="text-xs text-muted-foreground">Loading storage stats…</span>
                  </div>
                )}
              </div>
