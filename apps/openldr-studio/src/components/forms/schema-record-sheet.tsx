...
const SchemaRecordSheet = ({
  ...
  const onError = (errors: any) => {
    if (isOpen && errors && Object.keys(errors).length > 0) {
      toast.custom((id) => (
        <div className="flex rounded-lg border border-[#ff4500] bg-[#b33000] text-foreground-default shadow-lg ring-1 ring-[#74d0408] w-full md:max-w-91 p-4">
          ...
          <div className="shrink-0 max-w-8">
            <button
              className="flex items-center h-4 w-4 justify-center rounded-[56px] bg-transparent font-semibold hover:bg-[#ffffff] hover:text-[#000000]"
              onClick={(e) => {
                e.preventDefault();
                toast.dismiss(id);
              }}
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="flex flex-col min-w-160 p-0 pt-2 gap-0 "
      ...
      >
        ...
        <button className="opacity-0 w-0 h-0 pointer-events-none">
        ...
        {data?.columns && data?.columns.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center h-full w-full relative">
          ...
          <div className="flex flex-1 w-full overflow-y-auto px-3 overflow-x-hidden">
            <ErrorBoundary>
              <AutoFormProvider>
                <SchemaRecordForm
                  ref={formRef}
