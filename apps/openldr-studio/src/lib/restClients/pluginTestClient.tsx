...
export const pluginTestApi = {
  ...
  runTest: async (
    ...
    if (file) {
      // Binary: send as multipart/form-data so the backend receives the raw file
      const form = new FormData();
      form.append("file", file);
      form.append("contentType", data.contentType ?? "application/octet-stream");
      if (data.validationPluginId) form.append("validationPluginId", data.validationPluginId);
      if (data.mappingPluginId) form.append("mappingPluginId", data.mappingPluginId);
      if (data.storagePluginId) form.append("storagePluginId", data.storagePluginId);
      if (data.outpostPluginId) form.append("outpostPluginId", data.outpostPluginId);

      const res = await fetch(
        `${ENV.VITE_PROCESSOR_BASE_URL}/api/v1/projects/run`,
        {
          method: "POST",
      ...
      if (!res.ok || json.ok === false)
        throw new Error(json.error ?? `HTTP ${res.status}`);
      return json as RunPluginTestResponse;
    }

    // Text: send as JSON (existing path)
    return request<RunPluginTestResponse>(
      token,
      "/run",
      {
        method: "POST",
