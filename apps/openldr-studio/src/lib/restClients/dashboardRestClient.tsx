...
export async function getInfraDashboard(
  ...
  Pick<DashboardData, "pipeline" | "services" | "databases">
> {
  const qs = buildQueryParams(filters);
...
/**
 * Fetch storage stats separately — slow due to MinIO object listing.
 */
export async function getInfraStorage(
  token: string,
  signal?: AbortSignal,
): Promise<Pick<DashboardData, "storage">> {
  const data = await apiFetch(
    `/api/v1/dashboard/infrastructure/storage`,
    token,
    signal,
  );
  return { storage: data as DashboardData["storage"] };
}
