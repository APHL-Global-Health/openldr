...
//Import Custom types
import { OptionsHandler } from "../custom/primitve/options";
...
export function convertJsonSchemaToZod(
  ...
  if (types.file !== undefined) {
    allowedSchemas.push(types.file || $ZodFile({} as any));
  }
  if (types.options !== undefined) {
    allowedSchemas.push(types.options || $ZodOptions({}));
  }
  if (types.date !== undefined) {
    allowedSchemas.push(types.date || $ZodDate());
  }
  ...
  if (types.object !== false && types.object !== undefined) {
    if (types.object) {
  ...
  if (allowedSchemas.length === 0) {
    ...
    if (!hasConstraints) {
      ...
      zodSchema = z.union(
        allowedSchemas as [z.ZodTypeAny, z.ZodTypeAny, ...z.ZodTypeAny[]],
      );
    }
  }
