...
export class PrefixItemsHandler implements RefinementHandler {
  apply(zodSchema: z.ZodTypeAny, schema: JSONSchema.BaseSchema): z.ZodTypeAny {
    ...
    if (
      ...
      Array.isArray((arraySchema as any).prefixItems)
    ) {
      const prefixItems = (arraySchema as any).prefixItems;
      const prefixSchemas = prefixItems.map((itemSchema: any) =>
        convertJsonSchemaToZod(itemSchema),
      );
      ...
      return zodSchema.refine(
        (value: any) => {
          ...
          for (
            let i = 0;
            i < Math.min(value.length, prefixSchemas.length);
            i++
          ) {
          ...
          if (value.length > prefixSchemas.length) {
            if (
              ...
              const additionalItemSchema = convertJsonSchemaToZod(
                arraySchema.items,
              );
