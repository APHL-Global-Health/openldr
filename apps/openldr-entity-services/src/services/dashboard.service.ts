...
// Cache storage stats for 5 minutes — listing 600k+ objects per call is too slow
let storageCache: { data: any; expiresAt: number } | null = null;
const STORAGE_CACHE_TTL_MS = 5 * 60 * 1000;

export async function getStorageOverview(intClient?: PoolClient): Promise<any> {
  if (storageCache && Date.now() < storageCache.expiresAt) {
    return storageCache.data;
  }

  // Acquire our own connection if none was provided (standalone endpoint)
  const ownClient = intClient ? null : await pool.connect();
  const client = intClient || ownClient!;
  ...
  try {
    ...
    storageCache = { data: result, expiresAt: Date.now() + STORAGE_CACHE_TTL_MS };
    return result;
  } catch (error) {
...
export async function getInfraDashboard(
  ...
  try {
    ...
    return { pipeline, services, databases };
  } finally {
    intClient.release();
    extClient.release();
  }
