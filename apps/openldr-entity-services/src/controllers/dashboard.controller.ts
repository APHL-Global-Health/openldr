...
// ============================================================
// Infrastructure – Storage Only (slow, fetched separately)
// ============================================================

router.get("/infrastructure/storage", async (req: Request, res: Response) => {
  const start = Date.now();
  try {
    const data = await getStorageOverview();
    logger.info(
      { durationMs: Date.now() - start },
      "Dashboard response: infrastructure/storage"
    );
    return res.json(data);
  } catch (error: any) {
    logger.error(
      { error: error.message, durationMs: Date.now() - start },
      "Dashboard error: infrastructure/storage"
    );
    return res.status(500).json({
      success: false,
