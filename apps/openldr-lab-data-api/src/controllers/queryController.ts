  ...
  try {
    ...
    if (!concept_code && !to_concept_code && !to_source_name) {
      return res.status(400).json({
        error: 'Bad Request',
        message: 'At least one mapping parameter is required (concept_code, to_concept_code, or to_source_name)',
        status_code: 400,
        timestamp: new Date().toISOString()
      });
    }
    ...
    let params = [];
    let paramCount = 0;
    const concept_codeStr = typeof concept_code === 'string' ? concept_code : '';
    ...
    if (to_concept_code) {
      paramCount++;
      jsonbConditions.push(`mappings @> $${paramCount}`);
      params.push(JSON.stringify({ [concept_codeStr]: { mappings: [{ toConceptCode: to_concept_code }] } }));
    }

    if (to_source_name) {
      paramCount++;
      jsonbConditions.push(`mappings @> $${paramCount}`);
      params.push(JSON.stringify({ [concept_codeStr]: { mappings: [{ toSourceName: to_source_name }] } }));
    }
...
router.get('/facility-comparison', async (req, res) => {
  try {
    ...
    const _facility_codes = typeof facility_codes === 'string' ? facility_codes : '';
