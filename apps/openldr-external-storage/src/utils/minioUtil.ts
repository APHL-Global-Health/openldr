...
if (
  !process.env.EXTERNAL_STORAGE_MINIO_ACCESS_KEY ||
  !process.env.EXTERNAL_STORAGE_MINIO_SECRET_KEY
) {
...
const minioClient = new Minio.Client({
  ...
  port: 9000,
  accessKey: process.env.EXTERNAL_STORAGE_MINIO_ACCESS_KEY,
  secretKey: process.env.EXTERNAL_STORAGE_MINIO_SECRET_KEY,
  useSSL: false,
});
...
async function statObject({
  ...
  try {
    ...
    console.error(
      `Error getting object stats for ${objectName} from bucket ${bucketName}:`,
      error
    );
    throw error;
  }
...
export { getObject, statObject, putObject };
