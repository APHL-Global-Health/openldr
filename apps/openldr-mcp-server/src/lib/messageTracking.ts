...
export async function markStageCompleted(params: {
  ...
  await pool.query(
    `UPDATE "messageProcessingRuns" SET ${updates.join(
      ", ",
    )} WHERE "messageId" = $1`,
    values,
  );
...
export function extractTrackingContextFromKafkaValue(
  ...
  return {
    ...
    objectPath:
      bucketName && decodedKey ? `${bucketName}/${decodedKey}` : decodedKey,
  };
}
