...
pnpm --filter @openldr/mcp-server copy:env

# Start the server
pnpm --filter @openldr/mcp-server start
```
...
2. **installer** -- Installs dependencies with `pnpm install --frozen-lockfile`, copies the pruned workspace, and builds.
3. **runner** -- Copies the built output and runs `pnpm start`.
...
When running the inspector locally with `pnpm inspect`, it expects the MCP server to be running on `localhost:6060`. When running via Docker Compose, ensure both containers are on the same network and use the service hostname `openldr-mcp-server`.
